...
if GetResourceState('yflip-phone') == 'started' then groupSystem = 'yflip' end
...
elseif groupSystem == 'yflip' then
    ...
    print('710-lib: Detected YFlip-Phone')
    RegisterNetEvent('710-lib:Groups:PS:AddItem', function(item, count)
        local source = source
        local Player = Framework.PlayerDataS(source)
        Player.AddItem(item, count)
    ...
    local Groups = {}
    local function checkIfInGroup(source)
        local source = source
        local Player = Framework.PlayerDataS(source)
    ...
    local function manageExistingGroup(source, Pid, isLeader)
...
Framework.Group.AddItem = function(groupid, item, count)
    ...
    elseif groupSystem == 'yflip' then
        exports["yflip-phone"]:SendGroupEvent(groupid, '710-lib:Groups:PS:AddItem', {item = item, count = count})
    else
...
Framework.Group.CheckIfInGroup = function(source)
    if groupSystem == 'yflip' then
        return exports["yflip-phone"]:FindGroupByMember(source)
...
Framework.Group.SendNoti = function(groupid, message)
    ...
    elseif groupSystem == 'yflip' then
        exports["yflip-phone"]:NotifyGroup(groupid, message, 10000)
    else    
...
Framework.Group.AddMoney = function(groupid, type, amount)
    ...
    elseif groupSystem == 'yflip' then
        exports["yflip-phone"]:SendGroupEvent(groupid, '710-lib:Groups:PS:AddMoney', {type = type, amount = amount})
    else
        --- No group system detected so we use 710-lib's built in group system
